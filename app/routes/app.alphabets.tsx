...
async function editToDatabase(formData: FormData, responseData: any) {
  ...
  const data = {
      ...
      height: patchheight,
      width: patchWidth,
      alphabetname: patchName,
     
      // Add other properties from your Prisma schema here
  };

  try {
    const lolaAlphabets= await db.lolaAlphabets.update({
      where: { id: id },
      data
    });
...
async function saveToDatabase(formData: FormData, responseData: any) {
  ...
  try {
      const lolaAlphabets = await db.lolaAlphabets.create({
          data,
          // If you have a select option, provide it here
      });
      return json({ success: true, ImageData: lolaAlphabets });
  } catch (error) {
...
/>
        ...
        alphabets.map((alphabet: Patches) => (
          ...
          <tr key={alphabet.id} className={styles.patchparent}>
            ...
            <div className={styles.patchImageDiv}>
              <img src={alphabet.image}  className={styles.patchImage} />
            </div>
            <p>{alphabet.alphabetname}</p>
          </div>
            </td>
  ...
  : <img src={alphabet.image} className={styles.patchproductimage} />
          ...
          <FormField
            htmlFor="title"
            label="Title"
            value={formData.title || alphabet.alphabetname} 
            onChange={(e) => setFormData({ ...formData, title: e.target.value })}
          />
          <FormField
            htmlFor="patchHeight"
